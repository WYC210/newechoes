  ...
  useEffect(() => {
    ...
    const initThreeScene = async () => {
      try {
        ...
        const getColors = () => {
          return {
            earthBase: isDarkMode ? "#1e293b" : "#2a4d69", // Use lighter surface color for better contrast
          ...
          return function (this: any, ...args: any[]) {
            ...
            if (!inThrottle) {
              ...
              timeoutId = window.setTimeout(() => {
                inThrottle = false;
                // 如果在节流期间有新的调用，且距离上次运行超过限制，执行最后一次
                if (lastArgs && lastRan && Date.now() - lastRan >= limit) {
                  // 这里可以选择是否执行这最后一次，或者让它自然结束
                  // 为了平滑，通常不在这里执行，而是依赖下一次调用。
                  // 但为了捕获停止移动时的最后状态，应该执行。
                  // 不过原逻辑比较复杂，我们简化为标准节流：
                }
              }, limit);
            } else {
              ...
              const remaining = limit - timeSinceLastRan;

              timeoutId = window.setTimeout(() => {
                if (lastArgs) {
                  func.apply(context, lastArgs);
                  lastRan = Date.now();
                  // 这里的inThrottle会在下一个周期通过新的调用管理，或者需要重置
                  // 标准节流模式：
                  inThrottle = false;
                  // 注意：这可能导致连续触发。更稳健的实现：
                  // 保持 inThrottle = true 直到 limit 结束。
                  // 但上面的实现有点混合。
                }
        ...
        // 使用更可靠的节流实现 (Leading & Trailing)
        const createThrottle = (func: Function, limit: number) => {
          let lastFunc: number;
          let lastRan: number;
          return function (this: any, ...args: any[]) {
            const context = this;
        ...
        // 替换原来的 throttle
        const throttleV2 = (func: Function, limit: number) => {
        ...
        const onMouseMove = throttle((event: MouseEvent) => {
          ...
          // 强制渲染一帧，确保高亮效果立即显示（因为自动旋转暂停时渲染循环可能会跳过）
          if (sceneRef.current) {
        ...
        containerRef.current.addEventListener("mouseenter", onMouseEnter, { passive: true });
        containerRef.current.addEventListener("mouseleave", onMouseLeave, { passive: true });
