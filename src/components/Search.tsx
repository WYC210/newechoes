...
const Search: React.FC<SearchProps> = ({
  ...
  const performSearch = async (
    ...
    try {
      // 正确的逻辑
      const page = isLoadMore ? currentPage + 1 : 1;  // 加载更多时使用下一页
      ...
      let updatedItems;
      if (isLoadMore) {
        updatedItems = [...allItems, ...result.items];
      ...
      const hasMore = page < result.total_pages && updatedItems.length < result.total;


      setHasMoreResults(hasMore);

      if (isLoadMore) {
        setCurrentPage(page);  // 直接设置为当前请求的页码
      }
      if (isLoadMore) {
        // 合并新结果，并去除重复项（基于ID）
